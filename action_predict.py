...
class NonVisualModel(ActionPredict):
    ...
    def get_model(self, data_params):
        ...
        for l in range(self.num_layers):
            ...
            CC_Attn_out = MultiHeadAttention(self.num_heads, self.hidden_dim // self.num_heads)(CC_Attn_input, CC_Attn_input)
            #CC_Attn_out = Dropout(self.dropout_rate)(CC_Attn_out)
            #CC_Attn_out = LayerNormalization()(CC_Attn_input + CC_Attn_out)

            # Update the global yellow CLS
            yellow_cls = CC_Attn_out[:, 0:1, :]

            # Replace each branch CLS with CC_Attn-updated CLS and apply FFN
            for i, t in enumerate(data_types):
                ...
                new_layer_input[t] = x
            # Prepare inputs for the next layer (sequence with updated CLS)
...
class NonVisualModel_v4(ActionPredict):
    ...
    def get_model(self, data_params):
        ...
        # Per-branch CLS (no yellow CLS)
        cls_layer = LearnableCLSTokens(data_types, self.hidden_dim)
        cls_tokens_dict, _ = cls_layer(tf.shape(inputs[0])[0])

        # First projection + prepend CLS  -> [B, 1+T, D]
        for t in data_types:
            x = Dense(self.hidden_dim)(layer_input[t])
            x = Dropout(self.dropout_rate)(x)
            layer_input[t] = tf.concat([cls_tokens_dict[t], x], axis=1)

        # IC-Attn per branch + mean-pool fusion of CLS
        fused_cls_last = None
        for l in range(self.num_layers):
            token_after_ic, ic_cls = {}, []
            for t in data_types:
                x = self.position_layer(layer_input[t])
                x = MultiHeadAttention(self.num_heads, self.hidden_dim // self.num_heads)(x, x)
                token_after_ic[t] = x
                ic_cls.append(x[:, 0:1, :])

            fused_cls = tf.reduce_mean(tf.concat(ic_cls, axis=1), axis=1, keepdims=True)

            new_layer_input = {}
            for t in data_types:
                branch_tokens = token_after_ic[t][:, 1:, :]
                x = tf.concat([fused_cls, branch_tokens], axis=1)
                x_res = x
                x = Dense(self.hidden_dim * 2, activation='relu')(x)
                x = Dense(self.hidden_dim)(x)
                x = Dropout(self.dropout_rate)(x)
            ...
            layer_input = new_layer_input
            fused_cls_last = fused_cls

        # Head
        final_cls = tf.squeeze(fused_cls_last, axis=1)
        out = Dense(self.hidden_dim, activation='relu')(final_cls)
        out = Dropout(0.2)(out)
        out = Dense(1, activation='sigmoid')(out)
        return Model(inputs=inputs, outputs=out)
...
class NonVisualModel_v5(ActionPredict):
    ...
    def get_model(self, data_params):
        ...
        # Per-branch CLS + yellow CLS
        cls_layer = LearnableCLSTokens(data_types, self.hidden_dim)
        cls_tokens_dict, yellow_cls = cls_layer(tf.shape(inputs[0])[0])

        # First projection + prepend CLS
        for t in data_types:
            x = Dense(self.hidden_dim)(layer_input[t])
            x = Dropout(self.dropout_rate)(x)
            layer_input[t] = tf.concat([cls_tokens_dict[t], x], axis=1)
        ...
        for l in range(self.num_layers):
            ...
            h_e = yellow_cls[:, 0, :]
            H = tf.concat(cls_tokens, axis=1)
            W_s = Dense(self.hidden_dim, use_bias=False, name=f"cc_attn_proj_{l}")
            scores = tf.matmul(tf.expand_dims(h_e, 1), W_s(H), transpose_b=True)
            alpha = tf.nn.softmax(tf.squeeze(scores, 1), axis=-1)
            h_c = tf.reduce_sum(tf.expand_dims(alpha, -1) * H, axis=1)
            W_c = Dense(self.hidden_dim, activation='tanh', name=f"cc_attn_out_{l}")
            V = W_c(tf.concat([h_c, h_e], axis=-1))
            yellow_cls = tf.expand_dims(V, axis=1)

            new_layer_input = {}
            for t in data_types:
                branch_tokens = token_after_ic[t][:, 1:, :]
                x = tf.concat([yellow_cls, branch_tokens], axis=1)
                x_res = x
                x = Dense(self.hidden_dim * 2, activation='relu')(x)
                x = Dense(self.hidden_dim)(x)
                x = Dropout(self.dropout_rate)(x)
